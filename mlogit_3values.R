...
chatModel <- mlogit(Success ~ 1 | Good_Mate + Funny + Gender + Sexual, data = mlChat, reflevel = "No response/Walk Off")
summary(chatModel)
...
##Likelihood ratio test : chisq = 231.95 (p.value = < 2.22e-16) => 전체 모형의 가능도비에 따라 이러한 변화는 유의하다
...
##Odds Ration는 Funny 점수가 1점 올라갈 때 전화번호를 얻을 확률이 1.63 높아짐
##7. GenderFemale:Get Phone Number
##no response와 get phone number를 유의미하게 예측(b=-1.99, p<0.001)
##Odds Ratio는 남성(0)에서 여성(1)으로 변화함에 따라 전화번호를 얻을 확률이 0.14 정도